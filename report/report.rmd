...
\begin{equation}
$$
{f_{\textbf{X}}(\boldsymbol{\textbf{x}})=\frac{\exp\{-\frac{1}{2}(\textbf{x}-\boldsymbol{\mu})^T\boldsymbol{\Sigma}^{-1}(\textbf{x}-\boldsymbol{\mu})\}}{\int_{\textbf{F}\textbf{x} + g \geq 0}\exp\{-\frac{1}{2}(\textbf{x}-\boldsymbol{\mu})^T\boldsymbol{\Sigma}^{-1}(\textbf{x}-\boldsymbol{\mu})\}d\textbf{x}}I(\textbf{F}\textbf{x} + g \geq 0)}
$$
\label{eq:tmvg}
\end{equation}


Where $X$ is a $d$-dimensional truncated gaussian random variable, $I$ is an indicator function, and $\textbf{F}$ is an $m \times d$ matrix, which, together with the $m \times 1$ vector of $g$, defines $m$ constraints.  

We can rewrite $f_{\textbf{X}}$ as:   

\begin{equation}
f_{\textbf{X}}(\textbf{x}) = \frac1Z\exp\bigg\{-\frac{1}{2}\textbf{x}^T \Lambda \textbf{x} + \nu^T\textbf{x}\bigg\}I(\textbf{F}\textbf{x} + g \geq 0)
\label{eq:tmvg2}
\end{equation}

where $\Lambda = \boldsymbol{\Sigma}^{-1}$, $\nu = \boldsymbol{\Sigma}^{-1}\boldsymbol{\mu}$, and $Z$ is the normalizing constant.

## 2.2. Exact Hamiltonian Monte Carlo for Truncated Multivariate Gaussians   

Exact Hamiltonian Monte Carlo (HMC) for Truncated Multivariate Gaussians (TMG) (Pakman et al. (2013)) considers the exact paths of sample trajectories that follow the Hamiltonian equations of motion. Starting from $(\ref{eq:tmvg2})$, we can 




It takes the Hamiltonian function     

$$H = \frac{1}{2}\textbf{x}^T\textbf{x} + \frac{1}{2}\textbf{s}^T\textbf{s}$$
in which the joint distribution is $P(\textbf{X}, \textbf{S}) = \exp(-H)$. With this form of $H$, we can derive the Hamilton's equations as   
...
To sample $\textbf{X}$, we first sample values of $\textbf{S}$ from $\mathcal{N}(\textbf{0},\textbf{I}_d)$ and use them with previous values, $\textbf{x}$, as the initial conditions. We will then move along this trajectory for a time $T$ until we reach a new point $\textbf{x}^*$. This sequence forms a markov chain that converges to the distribution $P(\textbf{X})$ (Pakman et al. (2013)).   
