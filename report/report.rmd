...
Where $X$ is a $d$-dimensional truncated gaussian random variable, $I$ is an indicator function, and $\textbf{F}$ is an $m \times d$ matrix, which, together with the $m \times 1$ vector of $\textbf{g}$, defines $m$ constraints.  
...
Exact Hamiltonian Monte Carlo (HMC) for Truncated Multivariate Gaussians (TMG) (Pakman et al. (2013)) considers the exact paths of particle trajectories that follow Hamiltonian equations of motion:
...
To do this, let us sample from a standard normal distribution   
...
which can be obtained from $(\ref{eq:tmvg2})$ through linear change of variables.
